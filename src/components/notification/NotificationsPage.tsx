...
const mockNotifications: NotificationItem[] = Array.from({ length: 8 }).map((_, index) => ({
  id: `n-${index}`,
...
export default function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header: title, shared search/filters to the right, CTA */}
      <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4">
        <h1 className="text-lg font-medium text-gray-900">Notifications</h1>

        {/* Search + filters tray */}
        <div className="w-full md:ml-auto md:w-auto flex flex-col md:flex-row md:items-center border border-gray-200 rounded-md bg-white h-auto md:h-10 p-2 md:pl-2 md:pr-2 shadow-sm gap-2">
          <SearchBar 
            value={search} 
            onChange={setSearch} 
            placeholder=""
          ...
          <div className="flex items-center gap-2 md:ml-2 flex-wrap md:flex-nowrap overflow-x-auto w-full md:w-auto bg-white rounded-md p-2 md:bg-transparent md:p-0">
            <FilterDropdown options={simpleOptions} value={dateRange} onChange={setDateRange} placeholder="Date Range" />
            <FilterDropdown options={simpleOptions} value={user} onChange={setUser} placeholder="User" />
            <FilterDropdown options={simpleOptions} value={actionType} onChange={setActionType} placeholder="Action Type" />
            <FilterDropdown options={simpleOptions} value={asset} onChange={setAsset} placeholder="Asset" />
            <ResetButton onClick={() => { setSearch(''); setDateRange(''); setUser(''); setActionType(''); setAsset(''); }} />
          </div>
        </div>
        ...
        <button className="w-full md:w-auto md:ml-4 h-10 px-6 rounded-lg bg-[#B8892E] text-white text-[13px] font-medium hover:bg-[#a67927] inline-flex items-center justify-center gap-2 whitespace-nowrap">
          ...
          <svg
            ...
            aria-hidden="true"
          >
            <path d="M3 10l14-4v12l-14-4v-4z" />
            <path d="M7 18a3.5 3.5 0 0 1-7 0" />
          </svg>
        </button>
      </div>
